...
def binomial_american_option(spot_price, strike_price, time_to_expiry, risk_free_rate, volatility, number_of_steps, option_type='call'):
    ...
    # Up and down factors for the binomial model
    u = np.exp(volatility * np.sqrt(dt))  # Up factor
    d = 1 / u  # Down factor
    # Risk-neutral probability
    p = (np.exp(risk_free_rate * dt) - d) / (u - d)
    
    # Initialize asset prices at maturity (end of the tree)
    ST = np.zeros(number_of_steps + 1)
    for i in range(number_of_steps + 1):
        ST[i] = spot_price * (u ** (number_of_steps - i)) * (d ** i)
    
    # Initialize option values at maturity
    option_values = np.zeros(number_of_steps + 1)
    if option_type == 'call':
        option_values = np.maximum(0, ST - strike_price)  # Payoff for call option
    elif option_type == 'put':
        option_values = np.maximum(0, strike_price - ST)  # Payoff for put option
    else:
        raise ValueError("Invalid option type. Use 'call' or 'put'.")

    # Step back through the tree for American options
    for j in range(number_of_steps - 1, -1, -1):
        for i in range(j + 1):
            # Calculate the value of holding the option (as in European)
            hold_value = np.exp(-risk_free_rate * dt) * (p * option_values[i] + (1 - p) * option_values[i + 1])
            
            # Calculate the value of early exercise
            if option_type == 'call':
                early_exercise_value = max(0, spot_price * (u ** (j - i)) * (d ** i) - strike_price)
            elif option_type == 'put':
                early_exercise_value = max(0, strike_price - spot_price * (u ** (j - i)) * (d ** i))
            
            # Option price is the maximum of holding the option or exercising early
            option_values[i] = max(hold_value, early_exercise_value)
    
    return option_values[0]  # Option price at the root of the tree
